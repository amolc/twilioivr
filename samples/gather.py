...
def english():
    ...
    gather.pause(1)
    gather.say(
        "Por   ,,,,favor presione ,,,,dos para español",
        voice="Polly.Conchita",
        language="es-ES",
    )
    gather.pause(1)
    gather.say(
        "Veuillez appuyer,,,, sur trois ,,,,pour le français",
        voice="Polly.Celine",
        language="fr-FR",
    )
    gather.pause(1)
...
def start_english():
    ...
    gather.pause(2)
    gather.say(
        "Please press two for Customs duties or Charge back  , ,,,,,, ,,,,   ,,,,, "
    )
    gather.pause(2)
    gather.say("Please press three to schedule a pickup and pay for it! ")
    gather.pause(2)
    gather.say(
        "Please press four for connecting with a customer representative  , ,,,,,, ,,,,   ,,,,, "
    )
    resp.pause()
    resp.append(gather)
    resp.redirect("/voice")
...
def selectoptions():
    ...
    if "Digits" in request.values:
        ...
        if int(choice) == 1:
            ...
            gather.say(
                "Please       provide        your        shipping       number      to     know      the     custom charges.  "
            )
            resp.append(gather)
        elif int(choice) == 3:
            resp.pause(3)
            ...
            gather.say(
                "Please provide your account number or phone number followed by a pound sign. "
            )
            resp.append(gather)
...
def confirmshipping():
    ...
    if "Digits" in request.values:
        ...
        if len(choice) >= 5:
            resp.say(
                "Please wait while we fetch the results", voice="alice", language="en"
            )
            resp.pause(length=3)
        ...
        gather = Gather(finishOnKey="#", action="/confirmshipping", timeout=5)
        gather.say(
            "Please provide your shipping number followed by a pound sign.",
            voice="alice",
            language="en",
        )
        resp.pause(3)
...
def getshippingdetails():
    ...
    if shipped_by == "0":
        ...
        msg1 = (
            ...
            + shipmentamount
            + ", would you like to process to your payment now "
        )
...
def getcustomdetails():
    ...
    if shipped_by == "0":
        ...
        msg1 = (
            "There is a total balance of "
            + shipmentamount
            + ", for custom charges would you like to process to your payment now "
        )
        msg3 = "Please press one to accept or two to cancel"
        resp.say(msg, voice="alice", language="en")
        resp.say(msg1, voice="alice", language="en")
...
def confirmtiming():
    ...
    if "Digits" in request.values:
        ...
        if int(choice) == 1:
            ...
            gather.say(
                "Please put 2 digit for the month, 2 digit for the day and 4 digit for the year. As an example: For (May twelve twenty twenty two), please type  (zero, five,,, one, two,,, two, zero, two, two)"
            )
            gather.pause(3)
            resp.append(gather)
...
def confirmdate():
    ...
    if "Digits" in request.values:
        ...
        if len(pickupdate) > 5:
            ...
            gather.say(
                "Do you want your pickup to be schedule in the morning from Nine AM to twelve PM or in the afternoon from twelve PM to five PM?"
            )
            gather.say(
                "Please, press one for morning pickup OR two for afternon pickup."
            )
            resp.append(gather)
        else:
...
def confirmpickuptime():
    ...
    if "Digits" in request.values:
        ...
        if int(pickuptime) == 1 or int(pickuptime) == 2:
            url = (
                "/confirmpickuppayment?shippingid="
                + shippingid
                + "&pickupdate="
                + pickupdate
                + "&pickuptime="
...
def confirmpickuppayment():
    ...
    if "Digits" in request.values:
        ...
        if int(choice) == 1:
            url = (
                "/payment?shippingid=" + str(shippingid) + "&pay=" + str(shipmentamount)
            )
            print(url)
...
def confirmpayment():
    ...
    if len(PaymentError) > 0:
        resp.say(
            "Your transaction is not success, please wait while I am tranfering you to an Agent"
        )
    else:
        ...
        shipping_response = req.get(url)
        resp.say(
            "Your transaction is completed successfuly. We have send you the confirmation to your phone and the invoice to your email."
        )
        resp.say(
            "If you have any question please feel free to contact us back at any time. We appreciate your business. Good bye!"
        )
        resp.hangup()
    return str(resp)
